...
class TranslationService {
    ...
    async translateText(text, targetLanguage) {
        try {
            ...
            logger_1.logger.info({ textLength: text.length, targetLanguage }, 'Starting text translation');
            const [translation] = await this.translate.translate(text, {
            ...
            logger_1.logger.error({ error, text: text.substring(0, 100), targetLanguage }, 'Error translating text');
            throw new Error('Failed to translate text');
        }
    }
    /**
    ...
    async textToSpeech(text, languageCode, voiceName) {
        try {
            ...
            logger_1.logger.info({ languageCode }, 'Fetching available voices');
            ...
            if (!selectedVoice) {
                ...
                const safeVoices = availableVoices.filter(v => v.name?.includes('Wavenet') &&
                ...
                if (safeVoices.length > 0) {
                    ...
                    const fallbackVoices = availableVoices.filter(v => !v.name?.includes('Neural2') &&
                        !v.name?.includes('Chirp') &&
                        !v.name?.includes('Aoede') &&
                        !v.name?.includes('Echo') &&
                        !v.name?.includes('Fable') &&
            ...
            logger_1.logger.info({ request }, 'TTS request details');
    ...
    async getAvailableVoices(languageCode) {
        try {
            ...
            logger_1.logger.error({ error, languageCode }, 'Error getting available voices');
     ...
     */
    async translateAndSpeak(hausaText, targetLanguage, voiceName) {
        try {
            logger_1.logger.info({ hausaText: hausaText.substring(0, 50), targetLanguage, voiceName }, 'Starting translate and speak pipeline');
            // Step 1: Translate text
            const translatedText = await this.translateText(hausaText, targetLanguage);
            logger_1.logger.info({ translatedText: translatedText.substring(0, 50) }, 'Translation completed');
            ...
            logger_1.logger.info({ audioSize: audioBuffer.length }, 'TTS completed');
            ...
            logger_1.logger.info({ usedVoice, languageCode }, 'Translate and speak pipeline completed successfully');
            return {
                translatedText,
                audioBuffer,
                languageCode,
