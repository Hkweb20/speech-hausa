...
export const summarizeText = async (req: Request, res: Response) => {
  try {
    ...
    logger.error({ error, userId: req.user?.id }, 'Error in summarizeText');
    res.status(500).json({
      success: false,
...
export const formatForPlatform = async (req: Request, res: Response) => {
  try {
    ...
    logger.error({ error, userId: req.user?.id, platform: req.body.platform }, 'Error in formatForPlatform');
    res.status(500).json({
      success: false,
...
export const formatForAllPlatforms = async (req: Request, res: Response) => {
  try {
    ...
    logger.error({ error, userId: req.user?.id }, 'Error in formatForAllPlatforms');
    res.status(500).json({
      success: false,
...
export const getAIUsage = async (req: Request, res: Response) => {
  try {
    ...
    logger.error({ error, userId: req.user?.id }, 'Error getting AI usage');
    res.status(500).json({
      success: false,
