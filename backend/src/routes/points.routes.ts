...
import { POINTS_ACTIONS } from '../config/subscription';
import { authenticate, AuthenticatedRequest } from '../middleware/mongodb-auth';
import { requirePoints } from '../middleware/premium.guard';
...
router.get('/balance', authenticate as any, async (req: any, res) => {
  try {
    ...
    logger.error({ error, userId: req.user?.id }, 'Error getting points balance');
    res.status(500).json({
...
router.get('/actions', (req, res) => {
  try {
    ...
    logger.error({ error }, 'Error getting points actions');
    res.status(500).json({
      error: 'Internal server error',
      code: 'INTERNAL_ERROR'
    });
...
router.post('/watch-ad', authenticate as any, async (req: any, res) => {
  try {
    ...
    const result = await usageService.addPointsFromAd(
      req.user!.id, 
      adId, 
      10 // 10 points per ad
    );
    ...
    logger.error({ error, userId: req.user?.id }, 'Error adding points from ad');
    res.status(500).json({
      error: 'Internal server error',
      code: 'INTERNAL_ERROR'
    });
...
router.post('/summarize', 
  ...
  async (req: any, res) => {
    try {
      ...
      logger.error({ error, userId: req.user?.id }, 'Error generating summary');
      res.status(500).json({
        error: 'Internal server error',
        code: 'INTERNAL_ERROR'
      });
...
router.post('/punctuation', 
  ...
  async (req: any, res) => {
    try {
      ...
      logger.error({ error, userId: req.user?.id }, 'Error fixing punctuation');
      res.status(500).json({
        error: 'Internal server error',
        code: 'INTERNAL_ERROR'
      });
...
router.post('/translate', 
  ...
  async (req: any, res) => {
    try {
      ...
      logger.error({ error, userId: req.user?.id }, 'Error translating content');
      res.status(500).json({
        error: 'Internal server error',
        code: 'INTERNAL_ERROR'
      });
