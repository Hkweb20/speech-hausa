...
import { mongoTranscriptsRepo, SearchFilters } from '../repositories/mongodb-transcripts.repository';
import { asyncHandler } from '../utils/asyncHandler';

// Get recent transcript history for a user
export const getRecentTranscripts = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const result = await mongoTranscriptsRepo.listByUser(userId, page, limit);
...
export const searchTranscripts = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (source) filters.source = source as 'live' | 'file_upload';
  if (dateFrom) filters.dateFrom = new Date(dateFrom as string);
  if (dateTo) filters.dateTo = new Date(dateTo as string);
  if (hasTranslation !== undefined) filters.hasTranslation = hasTranslation === 'true';
  if (tags) filters.tags = (tags as string).split(',');

  try {
    const result = await mongoTranscriptsRepo.searchByUser(
      userId, 
      (query as string) || '', 
      filters, 
      page, 
...
export const getTranscriptById = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  const { id } = req.params;
  ...
  try {
    const transcript = await mongoTranscriptsRepo.get(id);
...
export const updateTranscript = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  const { id } = req.params;
  ...
  try {
    ...
    const existingTranscript = await mongoTranscriptsRepo.get(id);
    ...
    const updatedTranscript = await mongoTranscriptsRepo.update(id, updates);
...
// Delete a transcript
export const deleteTranscript = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  const { id } = req.params;
  ...
  try {
    ...
    const deleted = await mongoTranscriptsRepo.remove(id);
...
export const exportTranscripts = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  ...
  try {
    const exportData = await mongoTranscriptsRepo.exportUserTranscripts(userId, format as 'json' | 'txt' | 'csv');
...
export const getTranscriptStats = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userId = req.user?.id;
  ...
  try {
    ...
    const recentTranscripts = await mongoTranscriptsRepo.getRecent(userId, 50);
