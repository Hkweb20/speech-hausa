...
export class MobileOptimizationService {
  ...
  static addCompressionHeaders(res: Response): void {
  ...
  static optimizeResponse(data: any, mobile: boolean = true): any {
  ...
  private static optimizeItem(item: any): any {
    ...
    if (optimized.transcripts && Array.isArray(optimized.transcripts)) {
      optimized.transcripts = optimized.transcripts.map((transcript: any) => ({
        id: transcript.id,
  ...
  static addMobileHeaders(res: Response): void {
  ...
  static createBatchResponse(operations: any[], results: any[]) {
    return {
      success: true,
      data: {
  ...
  static optimizePagination(page: number, limit: number, total: number) {
  ...
  static getCacheStrategy(endpoint: string) {
    const cacheStrategies: Record<string, { ttl: number; stale: number }> = {
  ...
  static logMobileRequest(req: any, res: any, responseTime: number) {
    const mobileInfo = {
    ...
    logger.info(mobileInfo, 'Mobile API request');
  }
}
